...
callback_playerlaststandzombies( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8 )
{
    ...
    if ( _id_3BFB( self, var_3, 1 ) && !isattackerhostzombie( var_1 ) && !isattackerbossozstage2( var_1 ) && !cananyplayertokenrevive() )
    {
    ...
    else
    {
        ...
        if ( maps\mp\agents\_agent_utility::_id_4054( self ) > 0 )
...
tryendgameafteragentsdie( var_0 )
{
    ...
    while ( maps\mp\agents\_agent_utility::_id_4054( self ) > 0 )
        wait 0.05;

    if ( !areanyotherplayersalive( self ) && !maps\mp\zombies\_util::_id_508F( self.selfreviveactive ) )
...
savelaststandweapons( var_0, var_1 )
{
    ...
    foreach ( var_4 in var_2 )
    {
        if ( !issubstr( var_4, "combatknife" ) )
            self._id_6F8D[self._id_6F8D.size] = var_4;
    }

    self._id_559C = self getcurrentprimaryweapon();

    if ( self._id_559C == "search_dstry_bomb_defuse_mp" && isdefined( self.lastnonuseweapon ) && isdefined( common_scripts\utility::array_find( self._id_6F8D, self.lastnonuseweapon ) ) )
        self._id_559C = self.lastnonuseweapon;

    if ( issubstr( self._id_559C, "killstreak" ) )
        self._id_559C = self._id_6F8D[0];

    if ( self._id_559C == "airdrop_sentry_marker_mp" )
        self._id_559C = self._id_6F8D[0];

    if ( self._id_559C == "none" )
        self._id_559C = self._id_6F8D[0];

    if ( issubstr( self._id_559C, "exo_suit_" ) )
        self._id_559C = self._id_6F8D[0];
    ...
    self.tombstoneweapon = self._id_559C;
    ...
    foreach ( var_9 in self._id_6F8D )
    {
...
{
    foreach ( var_1 in self._id_6F8D )
        self._id_6F8E[var_1]["fillMax"] = 1;
}
...
areanyalliedagentsalive( var_0 )
{
    ...
    foreach ( var_3 in var_1 )
    {
        ...
        if ( maps\mp\zombies\_util::_id_508F( var_3.fakeplayer ) )
            continue;
...
areanyotherplayersalive( var_0 )
{
    ...
    foreach ( var_3 in level.players )
    {
        ...
        if ( !maps\mp\zombies\_util::_id_5164( var_3 ) )
            continue;

        if ( maps\mp\zombies\_util::_id_5175( var_3 ) )
        {
            if ( maps\mp\agents\_agent_utility::_id_4054( var_3 ) == 0 )
                continue;
        }
...
maydolaststandzombies( var_0 )
{
    ...
    if ( maps\mp\zombies\_util::_id_508F( var_0.outofbounds ) )
        return 0;
...
laststandrevivezombie( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    self.reviveicon = createreviveicon( "hint_health_zm", 8, 8, ( 0.5, 1.0, 0.99 ) );
...
givebackweaponsfromlaststand()
{
    ...
    if ( issubstr( self._id_559C, "turrethead" ) )
    {
        var_2 = -1;

        for ( var_3 = 0; var_3 < self._id_6F8D.size; var_3++ )
        {
            var_4 = maps\mp\zombies\_wall_buys::getupgradeweaponname( self, self._id_6F8D[var_3] );
            var_5 = maps\mp\zombies\_util::getzombieweaponlevel( self, self._id_6F8D[var_3] );
    ...
    foreach ( var_7 in self._id_6F8D )
...
givestoredweapon( var_0, var_1 )
{
    ...
    if ( isdefined( self._id_6F8E[var_0]["fillMax"] ) )
    {
        self givemaxammo( var_0 );
        self._id_6F8E[var_0]["fillMax"] = undefined;
    }
    else
    {
        self setweaponammoclip( var_0, self._id_6F8E[var_0]["ammoclip"] );

        if ( common_scripts\utility::string_find( var_0, "akimbo" ) && isdefined( self._id_6F8E[var_0]["ammoclipleft"] ) )
            self setweaponammoclip( var_0, self._id_6F8E[var_0]["ammoclipleft"], "left" );
...
laststandupdatereviveiconcolorzombies( var_0, var_1 )
{
    ...
    self.reviveicon.color = ( 1.0, 0.5, 0.0 );
    maps\mp\_utility::playdeathsound();
    wait(var_1 / 3);
...
hostzombieupdateoutline()
{
    ...
    {
        if ( maps\mp\zombies\_util::_id_508F( var_8.terminal.terminaldeactivated ) )
            continue;

        if ( var_2.size )
        {
            if ( maps\mp\zombies\_terminals::perkterminalhostcureincooldown( var_8.terminal ) || maps\mp\zombies\_util::_id_508F( var_8.terminal.terminaldisabled ) )
...
spawnplayerhostzombie( var_0 )
{
    ...
    playfx( common_scripts\utility::getfx( "gib_full_body" ), var_1.origin, ( 1.0, 0.0, 0.0 ) );
    waitframe();
    var_2 = undefined;
    var_3 = undefined;
...
hostzombiehudvisionset( var_0 )
{
    ...
    if ( isdefined( level.zombieinfectedlightset ) )
        self lightsetoverridedisableforplayer();
}
