...
reward_givemoney( var_0 )
{
    ...
    foreach ( var_2 in level.players )
        var_2 maps\mp\gametypes\zombies::_id_41FB( var_0, undefined, 1 );
}
...
reward_weaponupgradethink()
{
    ...
    if ( self._id_A2E3[var_1]["level"] < 20 )
        maps\mp\zombies\_wall_buys::setweaponlevel( self, var_0, self._id_A2E3[var_1]["level"] + 1 );
    else if ( self._id_A2E3[var_1]["level"] == 20 )
        maps\mp\zombies\_wall_buys::setweaponlevel( self, var_0, 25 );
    else
        return;

    thread maps\mp\zombies\_zombies_audio::playerweaponupgrade( 0, self._id_A2E3[var_1]["level"] );
    self.numupgrades++;
}
...
penalty_droppods()
{
    ...
    if ( isdefined( level.carepackages ) )
    {
        foreach ( var_1 in level.carepackages )
            var_1 maps\mp\killstreaks\_airdrop::_id_2847( 1 );
    }
