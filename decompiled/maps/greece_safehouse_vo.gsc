...
scancameratargetdialogue()
{
    ...
    else if ( level.dialogcounter == 0 )
        level.dialogcounter++;
    else if ( level.dialogcounter == 1 )
        level.dialogcounter++;
    else
...
safehouseclearfirstfloordialogue()
{
    ...
    else if ( common_scripts\utility::flag( "FlagKVASafehousePatrollerDeath" ) )
    {
        wait 1;
...
{
    var_0 = maps\_utility::get_living_ai( "PacingGuard", "script_noteworthy" );

    if ( isdefined( var_0 ) )
    {
        var_0 endon( "death" );
        var_0 endon( "PacingGuardDamaged" );
        var_1 = 0;

        while ( isalive( var_0 ) )
        {
            wait 10.0;
            maps\_hms_utility::playdialog( level.dialogtable, "SafehouseSecondFloorTakingTooLong", undefined, "guy_alerted", var_0 );
            var_1++;
        ...
        wait 1.0;
        var_2 = common_scripts\utility::getstruct( "safehouse_pacing_npc", "targetname" );
        var_2 notify( "PacingGuard" );
    }
}
