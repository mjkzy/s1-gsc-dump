...
setup_common( var_0 )
{
    ...
    setup_objectives();
    var_1 = level.player _meth_831A();
    level.player _meth_830F( var_1 );
    thread corpse_cleanup();
    thread start_billboard_ads();
    level.df_player_rig_name = "viewbody_sentinel_pilot_mitchell_grapple";
...
delay_set_fall_damage()
{
    ...
    for (;;)
    {
        if ( getdvarint( "bg_falldamagemaxheight" ) != var_0 )
        {
            ...
            _func_0D3( "bg_falldamagemaxheight", var_0 );
...
player_died_water( var_0 )
{
    ...
    var_0 _meth_82B5( var_7, var_4 / 2 );
    level.player _meth_807F( var_0 );

    while ( var_0.angles[0] < 180 && var_0.angles[0] > 45 )
        wait 0.05;

    var_0 _meth_82AE( var_3, var_4, 0.05, var_4 - var_5 );
}
...
heli_start_shooting()
{
    ...
    for (;;)
    {
        foreach ( var_3 in var_1.mgturret )
        {
            ...
            var_3 _meth_8106( var_0, ( 0, 0, 36 ) );
            var_3 _meth_80E2();
            var_3 common_scripts\utility::delaycall( 2, ::_meth_80E3 );
        }
...
check_player_in_sniper_zone()
{
    ...
    foreach ( var_2 in var_0 )
    {
        if ( issubstr( var_2, "rsass_silenced" ) )
            level.player _meth_8315( "rsass_silenced" );
    }
}
    ...
    for (;;)
    {
        ...
        foreach ( var_2 in var_0 )
        {
            if ( issubstr( var_2, "iw5_mors_sp_morsscope" ) )
            {
                ...
                level.player _meth_8315( "iw5_mors_sp_morsscope" );
            }
        }
    ...
    for (;;)
    {
        ...
        if ( var_3.size )
        {
            ...
            for (;;)
            {
                ...
                if ( isdefined( self.enemy ) )
                {
                    var_5 = self.enemy _meth_8387();

                    if ( !isdefined( var_5 ) )
                        continue;

                    if ( _func_1FF( var_3[var_4], var_5 ) )
                    {
                        self _meth_81A6( var_3[var_4].origin );
